  ...
  return (
    ...
    >
      <Routes>
        ...
        <Route path="/players/:playerId" element={<PlayerDetailWrapper players={players} onPlayerUpdate={handleUpdatePlayerStorage} onDeletePlayer={handleDeletePlayer} onEditPlayerRedirect={(id) => navigate(`/players/edit/${id}`)} allPlayers={players} />} />
...
const PlayerDetailWrapper: React.FC<{players: Player[], onPlayerUpdate: () => Promise<void>, onDeletePlayer: (id: string) => Promise<void>, onEditPlayerRedirect: (id: string) => void, allPlayers: Player[]}> = ({players, onPlayerUpdate, onDeletePlayer, onEditPlayerRedirect, allPlayers }) => {
  const { playerId } = useParams<{ playerId: string }>();
  const navigate = useNavigate();
