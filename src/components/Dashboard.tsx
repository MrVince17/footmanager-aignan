    ...
    <div id="dashboard-content" className="space-y-8">
      <div className="bg-gradient-to-r from-red-600 to-black rounded-xl p-8 text-white relative overflow-hidden">
        <div className="absolute top-4 right-4">
          <button
            onClick={() => exportToPDF('dashboard-content', 'tableau_bord_US_Aignan.pdf')}
        ...
        <div className="relative z-10">
          ...
          <p className="text-red-100">Vue d'ensemble de votre équipe de football</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Répartition par Équipe</h3>
              ...
              <div className="flex items-center space-x-2">
                <div className="w-32 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(stats.seniors1Count / stats.totalPlayers) * 100}%` }}
                  ></div>
                </div>
                <span className="font-semibold text-gray-900">{stats.seniors1Count}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <div className="w-32 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(stats.seniors2Count / stats.totalPlayers) * 100}%` }}
                  ></div>
                </div>
                <span className="font-semibold text-gray-900">{stats.seniors2Count}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <span className="font-semibold text-gray-900">{stats.totalTrainings}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {adminIssues.length === 0 ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 text-red-600 mb-3">
              <AlertCircle size={20} />
              <span>{adminIssues.length} dossier(s) nécessite(nt) une attention</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Meilleurs Buteurs</h3>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Meilleure Assiduité Matchs</h3>
          <div className="space-y-3">
            {bestMatchAttendance.map((player, index) => (
              <div key={player.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold ${
                    index === 0 ? 'bg-red-600' : index === 1 ? 'bg-black' : 'bg-gray-500'
                  }`}>
                    {index + 1}
                  </div>
                  <span className="font-medium">{player.firstName} {player.lastName}</span>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Meilleure Assiduité Entraînements</h3>
          <div className="space-y-3">
